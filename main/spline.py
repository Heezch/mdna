...
class SplineFrames:
    ...
    def __init__(self, control_points, degree=3, closed=False, up_vector=[0, 0, 1],frame_spacing=0.34, frame_tolerance=0.5, verbose=False, num_points=1000, initial_frame=None):
        """
    ...
    def distribute_points_old(self):
        """
        Distributes points along the spline with a specified spacing.

        Args:
            d (float): Spacing between consecutive frames.

        Returns:
            self: Returns the instance of the class.
        """
        d = self.frame_spacing
        inter_distances = np.sqrt(np.sum(np.diff(self.curve, axis=0) ** 2, axis=1))
        self.arc_length = np.sum(inter_distances)
        cum_length = np.insert(np.cumsum(inter_distances), 0, 0)
        self.point_indices = np.unique(np.searchsorted(cum_length, np.arange(0, self.arc_length, d)))
        self._compute_frames()
        return self
    ...
    def _compute_frames_old(self):
        ...
        if self.initial_frame is not None:
            position, right, up, forward = self.initial_frame
            # origin, binormal to the curve (B), normal to the curve (N), tangent to the curve (T)
            self.frames.append((position, right, up, forward))

        for index in self.point_indices:
            position = self.curve[index]
            derivative = self.der1[index]
    ...
    def distribute_points(self):
        """
        Distribute points equidistantly along the spline with all segments the same size (as close as possible to 'd'),
    ...
    def plot_frames(self, fig=False,equal_bounds=False):
        """
    ...
    def twist_frames(self, bp_per_turn=10.5, modified_ranges=[], plot=False):
        self.twister = Twister(frames=self.frames, bp_per_turn=bp_per_turn, modified_ranges=modified_ranges, plot=plot, circular=self.closed)
        ...
        if self.closed:
            ...
            print(f"Structure is requested to be circular:\n Excess twist per base to make ends meet: {adj-(360/bp_per_turn):.2f} degrees")
...
class Twister:
    ...
    def rotate_basis(self, frame, twist_angle_degrees):
        ...
        # Directly modify the input fram
        frame[0] = B_rotated
        frame[1] = N_rotated
        return frame
